...
/**
 * The stock deck editor, with the one thing {@code QUEST_EDITOR_POOL} requires
 * of whoever uses it.
 *
 * That config sorts on {@link ColumnDef#NEW}, and NEW ships with null sort and
 * display functions — "functions will be set later" — so every consumer has to
 * supply them or {@code ItemColumn}'s constructor throws
 * "A sort function hasn't been set for column New". Quest does it by subclassing
 * the editor and overriding getColOverrides; so does Adventure; so, now, does
 * Chronicle. Pointing a config at a collection is not enough on its own.
 *
 * Supplying it is worth doing rather than dodging: in a collection game the
 * cards you just pulled are exactly the ones you want to find while you build,
 * so the catalog sorts by true acquisition order and badges what is new — the
