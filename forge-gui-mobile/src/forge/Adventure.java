...
public class Adventure implements Disposable {
    ...
    /**
     * Re-project the batches onto a new backbuffer size. SpriteBatch fixes its ortho projection at
     * construction, and the transition overlay draws in raw window pixels, so without this a resized
     * window fades through a launch-sized frame anchored in the bottom-left corner.
     */
    public void resize(int width, int height) {
        if (width <= 0 || height <= 0) { return; }
        final Matrix4 projection = new Matrix4().setToOrtho2D(0, 0, width, height);
        transitionBatch.setProjectionMatrix(projection);
        uiBatch.setProjectionMatrix(projection);
    }
